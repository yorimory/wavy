-- Полная миграция для БД, созданной до ролей и услуг (MySQL 8.0)
-- Можно выполнить вручную: docker exec -i wavy_mysql mysql -uwavy -pwavy_secret wavy_crm < server/sql/migrate_all.sql

ALTER TABLE users
  ADD COLUMN role ENUM('private_person', 'client') NOT NULL DEFAULT 'private_person' AFTER avatar_url;

ALTER TABLE appointments
  ADD COLUMN client_user_id BIGINT UNSIGNED NULL AFTER client_id,
  ADD KEY idx_appt_client_user (client_user_id);
