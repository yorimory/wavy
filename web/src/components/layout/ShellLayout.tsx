...
function MobileTab({
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex-1 flex flex-col items-center justify-center gap-0.5 h-[56px] transition-all ${
...
export function ShellLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-background flex-col lg:flex-row">
      ...
      <main
        className={
          (calendarFullBleed || location.pathname.startsWith("/messages"))
            ? "flex-1 h-[calc(100dvh-56px)] lg:h-screen w-full p-0 overflow-hidden flex flex-col bg-surface-container-low/30 pb-[calc(56px+env(safe-area-inset-bottom,0px))] lg:pb-0"
      ...
      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-start box-content"
        style={{
